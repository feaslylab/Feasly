-- Phase 11G — Linter-Silent, Self-Healing Security (Fixed - Exclude Vault)
...
BEGIN
  FOR r IN
    ...
    WHERE c.relkind = 'r'
      AND n.nspname NOT IN ('pg_catalog','information_schema','auth','vault','storage','realtime','supabase_functions')
  LOOP
    -- deny anon
    IF NOT EXISTS (
...
BEGIN
  ...
  LOOP
    IF r.schema_name NOT IN ('pg_catalog','information_schema','auth','vault','storage','realtime','supabase_functions') THEN
...
-- 11G.3 — Assert function for CI testing
...
BEGIN
  ...
  WHERE grantee IN ('anon','public')
    AND table_schema NOT IN ('pg_catalog','information_schema','auth','vault','storage','realtime','supabase_functions');

  IF leaks IS NOT NULL THEN
    RAISE EXCEPTION 'Grants to anon/public detected: %', leaks;
  END IF;

  -- B) function EXECUTE grants (exclude system schemas)
