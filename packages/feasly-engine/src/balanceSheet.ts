...
export function computeBalanceSheet(params: {
  ...
  const { T, revenue, depreciation, financing, tax, cash } = params;
  ...
  const carry = tax.vat_carry ?? z(T);
  const vatAsset = z(T), vatLiab = z(T);
  for (let t=0;t<T;t++){
    const v = carry[t] || new Decimal(0);
    vatAsset[t] = v.isNegative() ? v.abs() : new Decimal(0);
    vatLiab[t]  = v.isPositive() ? v       : new Decimal(0);
  }
  ...
  //    b) Retained earnings: plug to tie Assets = Liabilities + Equity
  const assetsTotal = z(T);
  const liabEqTotal = z(T);
  const re = z(T);
  let reCum = new Decimal(0);

  // Track tie-out diagnostic
  ...
  for (let t=0;t<T;t++){
    ...
    // Liabilities + Paid-in (no retained yet)
    const L_noRE = (debt[t]||new Decimal(0))
      .add(vatLiab[t]||0 as any)
      .add(paidIn[t]||0 as any);

    // Retained earnings as plug so A == L + RE
    const re_t = A.minus(L_noRE);
    reCum = re_t; // We store as level series equal to plug
    re[t] = reCum;

    assetsTotal[t] = A;
    liabEqTotal[t] = L_noRE.add(re[t]);
