...
import {
  ...
  numberifyTax, numberifyDep, numberifyCash, numberifyBalanceSheet, numberifyPnL, numberifyCashFlow, numberifyCovenants, numberifyWaterfall, numberifyEquity
  ...
  const num = useMemo(() => {
    try {
      return {
        ...
        equity: numberifyEquity((output as any).equity ?? {}),
        time: {
          df: output.time?.df ?? [],
          dt: output.time?.dt ?? []
        }
